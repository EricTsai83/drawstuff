    ...
    <DropdownMenuPrimitive.Portal>
      ...
      >
        <DropdownMenuPrimitive.Popup
          ...
          className={cn(
            "bg-popover text-popover-foreground z-50 max-h-(--available-height) min-w-[8rem] origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-md border p-1 shadow-md transition-[opacity,transform] outline-none data-ending-style:scale-95 data-ending-style:opacity-0 data-starting-style:scale-95 data-starting-style:opacity-0 data-[side=bottom]:data-starting-style:-translate-y-2 data-[side=left]:data-starting-style:translate-x-2 data-[side=right]:data-starting-style:-translate-x-2 data-[side=top]:data-starting-style:translate-y-2",
            className,
          )}
          {...props}
        />
...
function DropdownMenuItem({
  ...
  return (
    <DropdownMenuPrimitive.Item
      ...
      className={cn(
        // default 變體：僅在 focus/hover(highlight) 改變背景，不改變文字顏色
        "focus:bg-accent data-[highlighted]:bg-accent",
        // destructive 變體維持既有行為（文字為紅色，focus 背景略帶紅色）
        ...
        // 圖示預設為 muted，不隨 hover 改色；呼叫端可覆寫 text-*
...
function DropdownMenuSubContent({
  ...
  return (
    <DropdownMenuContent
      ...
      className={cn(
        "bg-popover text-popover-foreground w-auto min-w-[8rem] overflow-hidden rounded-md border p-1 shadow-lg",
        className,
      )}
      align={align}
      alignOffset={alignOffset}
