...
| T10 | `REALTIME_CRYPTO_VERSION` 升版讓既有密文不可讀 | **已緩解**（Plan 26，2026-08-06）：HKDF info 只剩 `drawstuff-key/${purpose}`，版本一律綁在各格式的 AAD；金鑰只隨 room generation 輪換 | 兩項殘留。(a) `COLLABORATION_PROTOCOL_VERSION` 仍在 snapshot／asset 的 **AAD 與 payload schema（`z.literal`）** 兩處，一次純 transport 升版仍會讓既有 durable 密文全部不可讀（但不再改變推導金鑰）→ [Plan 31](../../plans/31-durable-format-protocol-decoupling.md)。(b) **非靜默保證只由 snapshot 讀取這一個 oracle 承擔**：snapshot 解不開走 `unreadable-room`；realtime frame 解不開在 `relay-client.ts:312` 靜默丟棄，asset 解不開在 `asset-store.ts:466` 歸為 `abandon`——兩者在單一 frame／asset 層級都是刻意設計，但沒有任何地方聚合成「是金鑰錯了」。因此**尚無 stored snapshot 的 room 遇到錯誤金鑰時三條路徑全靜默**，使用者看到「已連線」卻永久空白 → [Plan 30](../../plans/30-silent-key-mismatch-detection.md)。（generation rotation 不受影響：`collaboration-session.ts:737` 由 server 端 `authGeneration` 比對偵測，回報 `generation-rotated`。） |
