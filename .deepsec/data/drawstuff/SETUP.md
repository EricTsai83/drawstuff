# Agent setup for `drawstuff`

This is a deepsec scanning workspace. Project `drawstuff` was just registered
(target: `..`). Setup is incomplete — `data/drawstuff/INFO.md`
...
2. **Fill in `data/drawstuff/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id drawstuff
pnpm deepsec process --project-id drawstuff
```
