 ...
 * verbatim, and a stored element may lack native fields (`groupIds`, `seed`,
 * `versionNonce`, `boundElements`, `updated`, `link`, `roundness`, `index`,
 * `frameId`) that upstream reads unguarded (e.g. `element.groupIds.length` in
 * its render/group/z-index code), so any consumer that passes decoded elements
 * to `excalidrawAPI.updateScene` crashes. `restoreScene` is upstream's own
 * load-time normalisation; running it here means every persisted read — owned
 * scene, shared link and published page alike — yields native elements.
 *
