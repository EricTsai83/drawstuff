...
import { isApplyResultAcceptable } from "@/hooks/whiteboard/use-apply-remote-scene";
...
type UseSceneRemoteRevisionCheckParams = {
  ...
  getActiveTheme?: () => "dark" | "light";
  workspaceId?: string;
  /** Composite readiness: true only when the engine is mounted and session state is fresh. */
  isReady: boolean;
